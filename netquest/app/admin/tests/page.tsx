...
const [loadingTests, setLoadingTests] = useState(true);
 ...
 const totalMarks = selectedQuestions.reduce((total, id) => {
  ...
  alert(`Selected Question IDs: ${selectedQuestions.join(", ")}`);
  console.log("SELECTED QUESTION IDs:", selectedQuestions);
  const testQuestionRows = selectedQuestions.map(
  ...
  console.log("TEST QUESTION ROWS:", testQuestionRows);
...
async function saveEditedTest() {
  ...
  const { data: updatedTest, error: testError } = await supabase
    ...
    .update({
      ...
      duration_minutes:
        Number(editingTest.duration_minutes) || 30,
      question_count: selectedQuestions.length,
  ...
  const newQuestionLinks = selectedQuestions.map(
    (questionId, index) => ({
...
async function toggleTestStatus(
  ...
  console.log("SUPABASE UPDATED TEST:", data);
...
async function deleteTest(testId: number, testTitle: string) {
  ...
  console.log("DELETED TEST FROM SUPABASE:", deletedTest);
